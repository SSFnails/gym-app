/**
 * Прогоняет тренировку насквозь в настоящем браузере: анкета профиля,
 * разминка, все подходы, связки, запас, итог. Печатает, что алгоритм
 * поставил на следующую сессию.
 *
 *   node scripts/walkthrough.mjs [url] [--rir=2] [--reps-short] [--sessions=2]
 */
...
try {
  ...
  while (steps++ < 400) {
    ...
    // Первый запуск в чистом профиле браузера начинается с анкеты.
    // Берём готовую программу: тогда дальше проверяется тот же путь,
    // что и раньше — тест подтягиваний, обязательная разминка, связки.
    if (body.includes('Откуда взять программу')) {
      await clickText('Взять готовую программу');
      await sleep(150);
      await clickText('ДАЛЬШЕ');
      log.push('анкета: взята готовая программа');
      await sleep(300);
      continue;
    }

    if (body.includes('Это первая прикидка')) {
      // Кнопка называется ровно «НАЧАТЬ» — на главной рядом живёт
      // «НАЧАТЬ ТРЕНИРОВКУ», поэтому ищем точное совпадение.
      await clickText('НАЧАТЬ', true);
      log.push('профиль создан');
      await sleep(1000);
      continue;
    }

    if (body.includes('ДАЛЬШЕ')) { await clickText('ДАЛЬШЕ'); await sleep(250); continue; }
