...
import { myStates } from './profiles.ts';
 ...
 */
export async function rollbackTwoWeeks(profileId: string): Promise<number> {
  const sessions = (await db.sessions.where('[profileId+status]').equals([profileId, 'done']).toArray())
  ...
  const states = await myStates(profileId);
